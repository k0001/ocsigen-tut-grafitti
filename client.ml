...
(* Contains all information wee need to stop interaction inside the page *)
...
let stop_drawing { drawing_thread; drawing_event_thread } =
  ...
  Lwt.cancel drawing_event_thread       (* also closes the bus *)
...
let launch_client_canvas bus image_elt canvas_elt =
  ...
  let drawing_event_thread =
    ...
    mousedowns canvas
      (fun ev _ -> set_coord ev;
        lwt () = line ev in
        Lwt.pick [mousemoves Dom_html.document (fun x _ -> line x);
                  Lwt.bind (mouseup Dom_html.document) line])
  in
  { drawing_thread = t; drawing_event_thread = drawing_event_thread }
